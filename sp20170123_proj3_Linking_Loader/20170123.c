...
int find_refaddr(int file_num, char* refername)
{
	exsymPtr temp;
	int refaddr=-1;
	for (int file_i = 0; file_i < file_num; file_i++) {
		for (temp = exsymTable[file_i]; temp != NULL; temp = temp->next)
		{
			if (strcmp(refername, temp->symname) == 0) {
				refaddr = temp->address;
				break;
			}
		}
	}
...
int main(void)
{
	...
	while(scanf("%s",command)!=EOF)
	{
		...
		case Loader: {
			...
			for (int i = 0; i < 3; i++) {
				file_list[i] = (char*)malloc(sizeof(char) * 100);
			}
			char a;
			while (1) {
				scanf("%s", buffer);
				...
				file_num++;
				strcpy(file_list[file_num - 1], buffer);
				a = getchar();
				if (a == '\n') break;
			}
			...
			//pass1
			csaddr = PROGADDR;
			exsymTable = malloc(sizeof(exsymPtr) * file_num);
			int file_i;
			char buf[255];
			char attrbuf[10];
			FILE *linkfp;
			...
			{
				//read file
				linkfp = fopen(file_list[file_i], "r");
				if(linkfp==NULL) {
				...
				while (1)
				{
					exsymPtr exnode = malloc(sizeof(exsymnode));
					exnode->ctrsection[0] = exnode->symname[0] = '\0';
					exnode->address = exnode->length = -1;
					exnode->next = NULL;
					//read line
					fgets(buf, 255, linkfp);
					if (feof(linkfp)) {
						break;
					}
					...
					//H record
					if (buf[0] == 'H') {
						strncpy(exnode->ctrsection, &(buf[1]), 6);
						exnode->ctrsection[6] = '\0';
						exnode->address = csaddr;
						sscanf(&(buf[13]), "%X", &(exnode->length));
						exsymTable[file_i] = exnode;
						temp = exsymTable[file_i];
					}
					if (buf[0] == 'D') {
						for (int x = 1;; x += 12) {
							exsymPtr exnode = malloc(sizeof(exsymnode));
							...
							strncpy(exnode->symname, &(buf[x]), 6);
							exnode->symname[6] = '\0';
							strcpy(exnode->symname, strip(exnode->symname));
							strncpy(attrbuf, &(buf[x + 6]), 6);
							sscanf(attrbuf, "%x", &(exnode->address));
							exnode->address += csaddr;
							//printf("%s %X\n", exnode->symname, exnode->address);
							temp->next = exnode;
							temp = temp->next;
						}
				...
				csaddr += (exsymTable[file_i]->length);
			}
			...
			for (file_i = 0; file_i < file_num; file_i++) {
				temp = exsymTable[file_i];
				printf("%-24s%04X         %04X\n", temp->ctrsection, temp->address, temp->length);
				while (1) {
					temp = temp->next;
					if (temp == NULL) break;
					printf("%18s      %04X \n", temp->symname, temp->address);

				}
			}
			printf("-------------------------------------------\n");
			PROGLEN = csaddr - PROGADDR;
			printf("%34s   %04X\n", "total length", PROGLEN);

			//pass2
			csaddr = PROGADDR;
			int referindex, Taddr, Tlen, Tvalue, Mvalue;
			char Tbuf[2];
			for (file_i = 0; file_i < file_num; file_i++)
			{
				//start of program;
				csaddr = exsymTable[file_i]->address;
				//read file
				linkfp = fopen(file_list[file_i], "r");
				...
				while (1)
				{
					...
					if (buf[0] == 'R') {
						//for reference
						for (int x = 1;; x += 8) {
							referPtr refer = malloc(sizeof(refernode));
							if (buf[x] == '\n') break;
							strncpy(attrbuf, &(buf[x]), 8);
							attrbuf[8] = '\0';
							referindex = atoi(attrbuf);
							if (referindex == 0) break;
							strncpy(refer->name, &(buf[x + 2]), 6);
							refer->name[6] = '\0';
							strcpy(refer->name, strip(refer->name));
							refer->address = find_refaddr(file_num, refer->name);
							refertable[referindex] = refer;
							if(buf[x+strlen(refer->name)+2]=='\n') break;
						}
						//reference for PROG-self (index 01)
						referPtr refer = malloc(sizeof(refernode));
						refer->address = exsymTable[file_i]->address;
						refertable[1] = refer;
					}
					//T record
					if (buf[0] == 'T') {
						strncpy(attrbuf, &(buf[1]), 6);
						sscanf(attrbuf, "%x", &Taddr);
						Taddr+=csaddr;
						strcpy(attrbuf, "       ");
						strncpy(attrbuf, &(buf[7]), 2);
						sscanf(attrbuf, "%x", &Tlen);
						for (int Ti = 0; Ti < Tlen; Ti++) {
							strncpy(Tbuf, &(buf[9 + Ti * 2]), 2);
							sscanf(Tbuf, "%x", &Tvalue);
							*(mem + Taddr + Ti) = Tvalue;
						}
					...
					if (buf[0] == 'M') {
						...
						strcpy(attrbuf, "      ");
						strncpy(attrbuf, &(buf[7]), 2);
						sscanf(attrbuf, "%d", &Tlen);
						strcpy(attrbuf, "      ");
						strncpy(attrbuf, &(buf[9]), 3);
						sscanf(attrbuf + 1, "%d", &Mvalue);
						if (Mvalue == 1) Mvalue = refertable[Mvalue]->address;
						else Mvalue = find_refaddr(file_num, refertable[Mvalue]->name);
						if (attrbuf[0] == '-') Mvalue = -Mvalue;
						if (Tlen == 5) {
							Tvalue = mem[Taddr + 1] * 0x100 + mem[Taddr + 2] + Mvalue;
							if (Tvalue > 0xF00000) {
								Tvalue = -(0xFFFFFF - Tvalue + 1);
							}
							int tmp;
							mem[Taddr + 1] = Tvalue / 0x100;
							tmp = Tvalue % 0x100;
							mem[Taddr + 2] = tmp;
						}
						if (Tlen == 6) {
							Tvalue = mem[Taddr] * 0x10000 + mem[Taddr + 1] * 0x100 + mem[Taddr + 2] + Mvalue;
							if (Tvalue < 0) Tvalue = 0xFFFFFF + Tvalue + 1;
							mem[Taddr] = Tvalue / 0x10000; mem[Taddr + 1] = (Tvalue / 0x100) % 0x100; mem[Taddr + 2] = Tvalue % 0x100;
						}
					}
				}
			}
		...
		case Run: {
			//reglist: 0-A, 1-X, 2-L, 3-B, 4-S, 5-T, 8-PC, 6-CC
			int pc = runaddr, n, i, x, b, p, e, lv, rv, target;
		
			PROGADDR = 0x00;	
			if (finishflag == 1) {
				printf("Error: Already ended Program\n");
				break;
			...
			while (1) {
				...
				if (finishflag == 0 && runaddr != pc) {

					for (int bi = 0; bi < bpnum; bi++) {
						if (bplist[bi] == pc) {
							runaddr = pc;
							stopflag = 1;
							break;
				...
				if (mem[pc] == 0xB4) {
					e = 0;
					reglist[mem[pc + 1] >> 0b100] = 0;
					reglist[8] += 2;
					pc += 2; continue;
				}
				//COMPR
				else if (mem[pc] == 0xA0) {
					if (reglist[mem[pc + 1] >> 0b100] < reglist[mem[pc + 1] & 0x0F]) reglist[6] = -1;
					if (reglist[mem[pc + 1] >> 0b100] == reglist[mem[pc + 1] & 0x0F]) reglist[6] = 0;
					else reglist[6] = 1;
					reglist[8] += 2; pc += 2; continue;
				}
				//TIXR
				else if (mem[pc] == 0xB8) {
					reglist[1]++;
					lv = reglist[1];
					rv = reglist[mem[pc + 1] >> 4];
					if (lv < rv) reglist[6] = -1;
					else if (lv > rv) reglist[6] = 0;
					else reglist[6] = 1;
					reglist[8] += 2; pc += 2; continue;
				}
				...
				if (e == 1) {
					reglist[8] += 4;
					target = mem[pc + 3] + mem[pc + 2] * 0x100;
				}
				//FORMAT 3
				else {
					reglist[8] += 3;
					target = mem[pc + 2] + (mem[pc + 1] & 0x0F) * 0x100;
				}
				//indexing
				if(b!=1 && (target>0xF00)&&e!=1) target = target-0xFFF-1;
				if (x == 1) target += reglist[1];
									
				if (b == 1) target += reglist[3];
				if (p == 1) target += reglist[8];

				//STL
				if (mem[pc] == 0x17) {
					mem[target] = reglist[2];
				}
				//LDB
				else if (mem[pc] == 0x69) {
					reglist[3] = target;
				...
				//LDA simple
					...
					reglist[0] = mem[target + 2] + mem[target + 1] * 0x100 + mem[target] * 0x10000;
				}
				//LDA immediate
				else if (mem[pc] == 0x01) {
					reglist[0] = target;
				...
				//COMP
					...
					if (i == 1) {
						if (target < reglist[0]) reglist[6] = 1;
						else if (target > reglist[0]) reglist[6] = -1;
						else reglist[6] = 0;
					}
				}
				//JEQ
				...
				//LDT simple
					...
					reglist[5] = mem[target + 2] + mem[target + 1] * 0x100 + mem[target] * 0x10000;
				}
				//TD
				else if (mem[pc] == 0xE3) reglist[6] = 1;
				//RD
				else if (mem[pc] == 0xDB) {
					reglist[0] = 0;
				}
				//STCH
					...
					mem[target] = reglist[0] & 0x0F;
				}
				//JLT
				else if (mem[pc] == 0x3B) {
					if (reglist[6] < 0) {
						reglist[8] = target;
						pc = reglist[8]; continue;
					}
				...
				//STX
					...
					if (n == 1 && i == 1) {
						mem[target + 2] = reglist[1] % 0x100;
						mem[target + 1] = (reglist[1] / 0x100) % 0x100;
						mem[target] = reglist[1] / 0x10000;
					}
				}
				//RSUB
				else if (mem[pc] == 0x4F) {
				...
				//STA
					...
					if (n == 1 && i == 1) {
						int val = reglist[0];
						mem[target + 2] = val % 0x100; val /= 256;
						mem[target + 1] = val % 256; val /= 256;
						mem[target] = val;
					}
				}
				//J simple
				...
				//J indirect
					...
					reglist[8] = mem[target + 2] + mem[target + 1] * 0x100 + mem[target + 2] * 0x10000;
					/*break;*/
				}
				//LDCH
				else if (mem[pc] == 0x53) {
					if (n == 1 && i == 1) {
						reglist[0] = mem[target];
					}
				}
				...
				//JUMP
				if (mem[pc] == 0x3F || mem[pc] == 0x4B || mem[pc] == 0x4F || mem[pc] == 0x33) {
					pc = reglist[8];
				}
			...
			if (finishflag == 1) {
				reglist[8] = PROGADDR + PROGLEN;
			}
			//print register
			printf("A : %06X ", reglist[0]);
			printf("X : %06X \n", reglist[1]);
			printf("L : %06X ", reglist[2]);
			printf("PC: %06X \n", reglist[8]);
			printf("B : %06X ", reglist[3]);
			printf("S : %06X \n", reglist[4]);
			printf("T : %06X \n", reglist[5]);
		...
		case Bp: {
			...
			//scan address and put into bplist
			else {
				scanf("%s", inputbuf);
